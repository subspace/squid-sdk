...
import {SpecId} from './interfaces/parsed-data'
...
export class RuntimeApi {
    ...
    getStorageItem(prefixOrQualifiedName: string, name?: string): StorageItem {
        let prefix: string
        if (name == null) {
            let [p, n] = this.splitQualifiedName(prefixOrQualifiedName)
            prefix = p
            name = n
        } else {
    ...
    private splitQualifiedName(name: QualifiedName): [prefix: string, name: string] {
    ...
    decodeExtrinsic(bytes: Bytes | Uint8Array): sub.Extrinsic {
